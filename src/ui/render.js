...
import { DEFAULT_HF_MODEL } from "../services/huggingFaceClient.js"
...
export function renderSettings(config, state, browserState = {}) {
  const byokStatus = browserState.status ?? STATUS.NOT_CONNECTED
  const byokModel = browserState.model ?? DEFAULT_HF_MODEL
  const byokResult = browserState.result ?? "No request has been sent from this tab."
  return `${pageHeading("CONTROL PLANE", "Settings", "Choose provider slots and freshness policy. BYOK mode below is browser-only and intentionally temporary.")}
    <div class="settings-layout">
      ...
      <section class="panel byok-panel"><div class="panel-header"><div><p class="panel-kicker">HUGGING FACE / BYOK</p><h2>Browser-only connection</h2></div>${statusPill(byokStatus)}</div><p class="byok-warning">INSECURE BROWSER-ONLY MODE: the token is used by this tab to call Hugging Face directly. It is not saved, but it is visible to this page while connected. Do not use this mode on a shared or untrusted device.</p><div class="settings-list"><label class="setting-field" for="hf-browser-token"><span><strong>Hugging Face token</strong><small>Enter a token with the minimum required inference permission. It will be cleared from the field after testing.</small></span><input id="hf-browser-token" type="password" autocomplete="new-password" spellcheck="false" placeholder="hf_..." /></label><label class="setting-field" for="hf-browser-model"><span><strong>Inference model</strong><small>Change only if your Hugging Face provider supports another chat model.</small></span><input id="hf-browser-model" type="text" value="${escapeHtml(byokModel)}" autocomplete="off" /></label></div><div class="byok-actions"><button class="button button-primary" id="hf-test-connect" type="button">Test &amp; Connect</button><button class="button button-secondary" id="hf-clear-token" type="button">Clear tab token</button></div><p class="byok-result" id="hf-result" aria-live="polite">${escapeHtml(byokResult)}</p></section>
      ...
      <section class="panel security-panel"><div class="panel-header"><div><p class="panel-kicker">SECURITY</p><h2>Secret boundary</h2></div>${statusPill(STATUS.READY)}</div><ul class="plain-list"><li>Normal provider settings never accept or store secrets.</li><li>BYOK token memory is cleared on refresh, tab close or Clear.</li><li>No BYOK token is written to localStorage, sessionStorage, URL or GitHub.</li><li>Static pages cannot hide a key that is intentionally used by browser JavaScript.</li></ul></section>
...
export function renderApp(route, state, config, browserState = {}) {
  const page = route === "events" ? renderEvents(state) : route === "historical" ? renderHistorical(state, config) : route === "sources" ? renderSources(state) : route === "settings" ? renderSettings(config, state, browserState) : renderDashboard(state, config)
